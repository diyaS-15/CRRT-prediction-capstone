...
def make_patient_level_split(
    ...
    # Caller is responsible for dropping NaN target rows before splitting; we
    # validate here so split indices line up with the df the caller will slice.
    if TARGET_COL in df.columns and df[TARGET_COL].isna().any():
        raise ValueError(
            f"{TARGET_COL} contains NaN rows. Drop them before calling "
            "make_patient_level_split so SplitResult indices align with df."
        )
    ...
    return SplitResult(train_idx=train_idx, val_idx=val_idx, test_idx=test_idx, summary=summary)
...
def get_Xy( df: pd.DataFrame, split: SplitResult,) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series,pd.Series,pd.Series]:
    # df must already have NaN target rows removed by the caller — split
    # indices were computed against this exact df, so re-dropping here would
