...
#
# CANONICAL pipeline: this is the model wired into serving/app.py and
# training/generate_report.py. It predicts the composite target
# TARGET_COL = "crrt_within_48h" (CRRT in first 24h OR 25-48h) using the
# shared FEATURE_COLS from features/preprocessing.py.
#
...
#
# Every run is logged to MLflow (params, metrics, artifacts) and the fitted
# pipeline is registered as a new version of the "crrt-xgb" model in the
# MLflow Model Registry. A new version starts in stage "None" — it does not
# automatically become the model serving/app.py loads. Run
...
def make_objective(preprocessor, X_train, y_train, groups, scale_pos_weight):
    """Optuna objective: mean average-precision across a GroupKFold CV,
    replacing the RandomizedSearchCV grid this script used previously —
    Optuna's TPE sampler picks each trial's hyperparameters based on what
    previous trials scored, converging faster than blind random sampling."""
...
def main():
    ...
    with mlflow.start_run(run_name="xgb"):
        ...
        mlflow.log_params({"data_path": data_path, "label_col": label_col, "group_col": group_col})
        ...
        study.trials_dataframe().to_csv("reports/xgb_optuna_trials.csv", index=False)
        artifact_paths.append("reports/xgb_optuna_trials.csv")
        print("Saved: reports/xgb_optuna_trials.csv")
        ...
        artifact_paths.append("reports/xgb_threshold_tuning.csv")
        ...
        plt.close("all")
        artifact_paths.append("reports/shap_summary.png")
        ...
        artifact_paths.append("reports/shap_values_test.csv")
        ...
        artifact_paths.append("reports/xgb_best_params.json")
        print("Saved: reports/xgb_best_params.json")
        # Save evaluation metrics
        with open("reports/xgb_metrics.json", "w") as f:
            json.dump(metrics, f, indent=2)
        artifact_paths.append("reports/xgb_metrics.json")
        print("Saved: reports/xgb_metrics.json")
        # Save trained model pipeline (kept alongside the MLflow-registered
        # copy below so serving/app.py's local-file fallback keeps working)
        ...
        artifact_paths.extend(["reports/val_predictions.csv", "reports/test_predictions.csv"])
        ...
        artifact_paths.append("reports/split_check.json")
        ...
        artifact_paths.append("reports/confusion_matrix.json")
        ...
        artifact_paths.extend([
            "reports/val_false_positives.csv",
            "reports/val_false_negatives.csv",
        ...
        artifact_paths.append("reports/xgb_feature_importance.csv")
        ...
        # Register this run's pipeline as a new (unpromoted) model version.
        # Use promote_model.py to compare it against Production and decide
        # whether it should replace what's actually being served.
        # cloudpickle instead of mlflow's newer default (skops), which refuses
        # to serialize XGBoost's internal Booster/XGBClassifier types as
        # "untrusted" -- fine here since we're both the producer and consumer.
        ...
        print(f"Registered model version under '{MLFLOW_MODEL_NAME}' (stage=None). "
              f"Run promote_model.py to move it to Production if it's better.")
